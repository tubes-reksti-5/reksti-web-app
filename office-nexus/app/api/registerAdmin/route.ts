...
export const POST = async (req: NextRequest) => {
  try {
    ...
    if (!user) {
      return NextResponse.json({ message: "Error", error: "Admin registration failed" }, { status: 400 });
    }
    ...
    return NextResponse.json({ message: "Admin registered successfully", user }, { status: 201 });
  } catch (err: unknown) {
